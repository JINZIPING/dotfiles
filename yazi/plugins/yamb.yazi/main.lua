...
local normalize_keys = function(bookmarks, keys)
	local used = {}
	for _, item in pairs(bookmarks) do
		if item.key and #item.key == 1 then
			used[item.key] = true
		end
	end

	for _, item in pairs(bookmarks) do
		if not item.key or #item.key ~= 1 then
...
return {
	setup = function(state, options)
		...
		normalize_keys(bookmarks, state.keys)
